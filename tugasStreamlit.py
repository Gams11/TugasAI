...
# --- 2. FITUR CACHING (PENTING AGAR TIDAK CRASH) ---
# Fungsi ini memastikan model diload sekali saja di memori server
@st.cache_resource
def load_model():
    # Pancing deepface untuk download/load weight di awal
    # Kita pakai gambar dummy kecil (hitam)
...
# Panggil fungsi caching ini saat aplikasi baru dibuka
with st.spinner("Sedang memuat model AI (Tunggu sebentar...)..."):
    load_model()
...
if mode == "Ambil Foto (Webcam)":
    ...
    # Menggunakan fitur native Streamlit Cloud
